/*Q107: Write a program to take an array arr[] of integers as input, the task is to find the 
previous greater element for each element of the array in order of their appearance in the array.
...
int main() {
    ...
    for(i = 0; i < n; i++) {
        ...
        // Check all elements on the left of arr[i]
        for(j = i - 1; j >= 0; j--) {
            if(arr[j] > arr[i]) {
                prev = arr[j];
